...
class SeatsPage extends StatefulWidget {
  ...
  const SeatsPage({super.key, required this.onSelectionChanged});
...
class _SeatsPageState extends State<SeatsPage> {
  late List<SeatsModel> seats;

  @override
  void initState() {
    super.initState();
    seats = List.generate(48, (i) {
      // أمثلة: خانات "فارغة" (مساحة) في منتصف الصفوف
      if ((i == 18) || (i == 21)) {
        return SeatsModel(id: i, status: SeatsStatus.empty);
      }
      // بعض الخانات محجوزة (ممتلئة باللون الأحمر)
      if (i >= 24 && i <= 31) {
        return SeatsModel(id: i, status: SeatsStatus.reserved);
      }
      if (i >= 32 && i <= 39) {
        return SeatsModel(id: i, status: SeatsStatus.reserved);
      }
      // الباقي متاحة
      return SeatsModel(id: i, status: SeatsStatus.available);
    });
  }
  ...
  void _onSeatTap(int index) {
    ...
    } else if (seat.status == SeatsStatus.selected) {
      seat.status = SeatsStatus.available;
    }
    final selectedCount = seats
        .where((s) => s.status == SeatsStatus.selected)
        .length;
    widget.onSelectionChanged?.call(selectedCount);
    setState(() {});
  }

  Color seatColor(SeatsStatus status) {
    switch (status) {
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return GestureDetector(
          onTap: () {
            if (seat.status != SeatsStatus.reserved &&
                seat.status != SeatsStatus.empty) {
              _onSeatTap(index);
            }
          },
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border: (seat.status == SeatsStatus.available)
